...
class ImportSales implements ToModel, WithStartRow
{
    ...
    public function model(array $row): ?Model
    {
        ...
        if (config('app.domain_name') == 'flex') {
            $this->column_array = ['pid', 'homeowner_id', 'proposal_id', 'customer_name', 'customer_address', 'customer_address_2', 'customer_city', 'customer_state', 'customer_zip', 'customer_email', 'customer_phone', 'setter_email', 'sales_rep_name', 'sales_rep_email', 'install_partner', 'install_partner_id', 'customer_signoff', 'm1_date', 'scheduled_install', 'install_complete_date', 'm2_date', 'date_cancelled', 'return_sales_date', 'gross_account_value', 'cash_amount', 'loan_amount', 'kw', 'dealer_fee_percentage', 'adders', 'cancel_fee', 'adders_description', 'funding_source', 'financing_rate', 'financing_term', 'product_code', 'epc', 'net_epc', 'job_status'];
        } elseif ($companyProfile->company_type == CompanyProfile::TURF_COMPANY_TYPE) {
            $this->column_array = ['pid', 'homeowner_id', 'proposal_id', 'customer_name', 'customer_address', 'customer_address_2', 'customer_city', 'customer_state', 'customer_zip', 'customer_email', 'customer_phone', 'setter_email', 'sales_rep_name', 'sales_rep_email', 'install_partner', 'install_partner_id', 'customer_signoff', 'm1_date', 'scheduled_install', 'install_complete_date', 'm2_date', 'date_cancelled', 'return_sales_date', 'gross_account_value', 'cash_amount', 'loan_amount', 'sq_ft', 'dealer_fee_percentage', 'adders', 'cancel_fee', 'adders_description', 'funding_source', 'financing_rate', 'financing_term', 'product_code', 'gross_sq_ft', 'net_sq_ft', 'location_code', 'job_status'];
        } else {
            $this->column_array = ['pid', 'homeowner_id', 'proposal_id', 'customer_name', 'customer_address', 'customer_address_2', 'customer_city', 'customer_state', 'customer_zip', 'customer_email', 'customer_phone', 'setter_email', 'sales_rep_name', 'sales_rep_email', 'install_partner', 'install_partner_id', 'customer_signoff', 'm1_date', 'scheduled_install', 'install_complete_date', 'm2_date', 'date_cancelled', 'return_sales_date', 'gross_account_value', 'cash_amount', 'loan_amount', 'kw', 'dealer_fee_percentage', 'adders', 'cancel_fee', 'adders_description', 'funding_source', 'financing_rate', 'financing_term', 'product_code', 'epc', 'net_epc', 'location_code', 'job_status'];
        }
        ...
        if ($this->row_num > 1 && $this->checkrowData($row)) {
            ...
            $m1 = isset($row[17]) && $row[17] != '' ? gmdate('Y-m-d', ((int) $row[17] - 25569) * 86400) : null;
            $m2 = isset($row[20]) && $row[20] != '' ? gmdate('Y-m-d', ((int) $row[20] - 25569) * 86400) : null;

            if (! empty($row[34])) {
                $product = ProductCode::select('product_id')->whereRaw('LOWER(product_code) = ?', [strtolower($row[34])])->first();

                if (! $product) {
                    $errors['columns'][] = 'Apologies, at row number '.$this->row_num.' it appears that product code does not exist.';
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, at row number '.$this->row_num.' it appears that product code does not exist.';
                    }
                } else {
                    $row[34] = $product->product_id;
                }
            }
            ...
            if ($sale && ! empty($row[21])) {
                ...
                if (! empty($row[21]) && is_string($row[21])) {
                    ...
                    $setterEmail = trim(strtolower($row[11]));
                    if (! empty($setterEmail) && ! empty($this->users[$setterEmail])) {
                        $setterId = isset($this->users[$setterEmail]) ? $this->users[$setterEmail] : null;
                    }

                    $userIds = [];
                    if ($sale->salesMasterProcess->setter1_id) {
                        $userIds[] = $sale->salesMasterProcess->setter1_id;
                    }
                    if ($sale->salesMasterProcess->setter2_id) {
                        $userIds[] = $sale->salesMasterProcess->setter2_id;
                    }
                    if ($sale->salesMasterProcess->closer1_id) {
                        $userIds[] = $sale->salesMasterProcess->closer1_id;
                    }
                    if ($sale->salesMasterProcess->closer2_id) {
                        $userIds[] = $sale->salesMasterProcess->closer2_id;
                    }
                    if ($sale && count($userIds) != 0) {
                        if (UserCommission::where(['pid' => $sale->pid, 'amount_type' => 'm1', 'status' => '3', 'settlement_type' => 'during_m2', 'is_displayed' => '1'])->whereIn('user_id', $userIds)->first()) {
                            $m1 = $sale->m1_date;
                        }
                        if (UserCommission::where(['pid' => $sale->pid, 'amount_type' => 'm2', 'status' => '3', 'settlement_type' => 'during_m2', 'is_displayed' => '1'])->whereIn('user_id', $userIds)->first()) {
                            $m2 = $sale->m2_date;
                        }
                    }
                    ...
                    foreach ($keys as $key => $column) {
                        if ($column != 'date_cancelled') {
                            if ($column == 'm1_date' || $column == 'm2_date' || $column == 'customer_signoff') {
                                ...
                                if ($setterId != $sale->salesMasterProcess->setter1_id) {
                                    $errors['columns'][] = 'Apologies, At row number '.$this->row_num." cancelling date detected. '".$column."' column data has changed. Verify all column data matches to the previous record.";
                                    if (! $this->validate_only) {
                                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." cancelling date detected. '".$column."' column data has changed. Verify all column data matches to the previous record.";
                                    }
                ...
                if (empty($row[16])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'customer_signoff' column is empty.";
                    }
                }
                ...
                if ($row[26] == '') {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'kw' column is empty.";
                    }
                } elseif ($row[26] != '' && ! is_numeric($row[26])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'kw' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'kw' column is 0 or less then 0 and the system can not accept it.";
                    }
                }
                ...
                if ($row[36] == '') {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'net_epc' column is empty.";
                    }
                } elseif ($row[36] != '' && ! is_numeric($row[36])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'net_epc' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'net_epc' column is 0 or less then 0 and the system can not accept it.";
                    }
                }
                // EPC IS MANDATORY
                if ($row[35] == '') {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'epc' column is empty.";
                    }
                } elseif ($row[35] != '' && ! is_numeric($row[35])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'epc' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'epc' column is 0 or less then 0 and the system can not accept it.";
                    }
                }
                ...
                if (! empty($row[16]) && is_string($row[16])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It appears that the date provided in the 'customer_signoff' column is not valid. Please make sure the date follows the correct format (Y-m-d) or (Y/m/d).";
                    }
                }
                ...
                if (! empty($row[17]) && is_string($row[17])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It appears that the date provided in the 'm1_date' column is not valid. Please make sure the date follows the correct format (Y-m-d) or (Y/m/d).";
                    }
                }
                ...
                if (! empty($row[20]) && is_string($row[20])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It appears that the date provided in the 'm2_date' column is not valid. Please make sure the date follows the correct format (Y-m-d) or (Y/m/d).";
                    }
                }
                // CANCLE DATE CHECKING FORMAT
                if (! empty($row[21]) && is_string($row[21])) {
                ...
                if (! empty($row[22]) && is_string($row[22])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It appears that the date provided in the 'return_sales_date' column is not valid. Please make sure the date follows the correct format (Y-m-d) or (Y/m/d).";
                    }
                }
                ...
                if (empty(trim($row[12]))) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'sales_rep_name' column is empty.";
                    }
                }
                ...
                if ($row[15] != '' && ! is_numeric($row[15])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'install_partner_id' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                }
                ...
                if ($row[24] != '' && ! is_numeric($row[24])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'cash_amount' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                }
                ...
                if ($row[25] != '' && ! is_numeric($row[25])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'loan_amount' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                }
                // DEALER FEE PERCENTAGE NUMERIC VALIDATION
                if ($row[27] != '' && ! is_numeric($row[27])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'dealer_fee_percentage' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                }
                // ADDERS NUMERIC VALIDATION
                if ($row[28] != '' && ! is_numeric($row[28])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'adders' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                }
                // CANCEL FEE NUMERIC VALIDATION
                if ($row[29] != '' && ! is_numeric($row[29])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'cancel_fee' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                }
                // FINANCING RATE VALIDATION
                if ($row[32] != '' && ! is_numeric($row[32])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'financing_rate' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                }
                // FINANCING TERM VALIDATION
                if ($row[33] != '' && ! is_numeric($row[33])) {
                    ...
                    if (! $this->validate_only) {
                        $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." It seems that the 'financing_term' column contain a value other then numbers. Please remove any non-numeric characters and try again.";
                    }
                }
                ...
                if ($sale) {
                    // CUSTOMER SIGNOFF DATE GOT REMOVED
                    if ((empty(gmdate('Y-m-d', ((int) $row[16] - 25569) * 86400)) && ! empty($sale->customer_signoff))) {
                        $errors['columns'][] = 'Apologies, At row number '.$this->row_num.' the Customer signoff date cannot be removed once it got set';
                        if (! $this->validate_only) {
                            $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num.' the Customer signoff date cannot be removed once it got set';
                        }
                    }

                    // CUSTOMER SIGNOFF DATE GOT CHANGED
                    if (! empty(gmdate('Y-m-d', ((int) $row[16] - 25569) * 86400)) && ! empty($sale->customer_signoff) && gmdate('Y-m-d', ((int) $row[16] - 25569) * 86400) != $sale->customer_signoff) {
                        $errors['columns'][] = 'Apologies, At row number '.$this->row_num.' the Customer signoff date cannot be changed once it got set';
                        if (! $this->validate_only) {
                            $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num.' the Customer signoff date cannot be changed once it got set';
                        }
                    }
                    ...
                    if ((empty(gmdate('Y-m-d', ((int) $row[17] - 25569) * 86400)) && ! empty($sale->m1_date))) {
                        if (UserCommission::where(['pid' => $sale->pid, 'amount_type' => 'm1', 'status' => '3', 'settlement_type' => 'during_m2', 'is_displayed' => '1'])->first()) {
                            ...
                            if (! $this->validate_only) {
                                $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num.' the M1 date cannot be changed because the upfront amount has already been paid';
                            }
                        }
                    }
                    ...
                    if ((empty(gmdate('Y-m-d', ((int) $row[20] - 25569) * 86400)) && ! empty($sale->m2_date))) {
                        if (UserCommission::where(['pid' => $sale->pid, 'amount_type' => 'm2', 'status' => '3', 'settlement_type' => 'during_m2', 'is_displayed' => '1'])->first()) {
                            ...
                            if (! $this->validate_only) {
                                $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num.' the M2 date cannot be changed because the M2 amount has already been paid';
                            }
                        }
                    }
                    ...
                    if (! empty(gmdate('Y-m-d', ((int) $row[17] - 25569) * 86400)) && ! empty($sale->m1_date) && gmdate('Y-m-d', ((int) $row[17] - 25569) * 86400) != $sale->m1_date) {
                        if (UserCommission::where(['pid' => $sale->pid, 'amount_type' => 'm2', 'status' => '3', 'settlement_type' => 'during_m2', 'is_displayed' => '1'])->first()) {
                            ...
                            if (! $this->validate_only) {
                                $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num.' the M1 date cannot be changed because the M2 amount has already been paid';
                            }
                        }
                    }
                    ...
                    if (empty($setterColumn) && empty($setterId) && ! empty($sale->salesMasterProcess->setter1_id)) {
                        ...
                        if (! $this->validate_only) {
                            $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." the setter field was previously populated with data, but it appears to be empty on the new sheet. Please fill the 'setter_email' column.";
                        }
                        ...
                        if (! $this->validate_only) {
                            $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num." the setter field was previously populated with data, but the current setter doesn't exists on our system. Please fill the valid 'setter_email'.";
                        }
                    }
                    ...
                    if (! empty($setterId) && ! empty($sale->salesMasterProcess->setter1_id) && $setterId != $sale->salesMasterProcess->setter1_id) {
                        if (UserCommission::where(['pid' => $sale->pid, 'amount_type' => 'm2', 'status' => '3', 'settlement_type' => 'during_m2', 'is_displayed' => '1'])->first()) {
                            ...
                            if (! $this->validate_only) {
                                $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num.' commission is already paid to the previous setter and therefore we can not remove the setter now.';
                            }
                        }
                    }
                    ...
                    if ($closerId != $sale->salesMasterProcess->closer1_id) {
                        if (UserCommission::where(['pid' => $sale->pid, 'amount_type' => 'm2', 'status' => '3', 'settlement_type' => 'during_m2', 'is_displayed' => '1'])->first()) {
                            ...
                            if (! $this->validate_only) {
                                $this->salesErrorReport[$row[0]][] = 'Apologies, At row number '.$this->row_num.' commission is already paid to the previous closer and therefore we can not remove the closer now.';
                            }
                        }
                    }
                    ...
                    if (config('app.domain_name') != 'flex') {
                        if (strtoupper($row[37]) != strtoupper($sale->location_code)) {
                            if (UserCommission::where(['pid' => $sale->pid, 'amount_type' => 'm2', 'status' => '3', 'settlement_type' => 'during_m2', 'is_displayed' => '1'])->first()) {
                                $errors['columns'][] = 'Apologies, at row number '.$this->row_num.", it seems that the 'location_code' column data has changed but the commission is already paid out, therefore we cannot change it now.";
                                if (! $this->validate_only) {
                                    $this->salesErrorReport[$row[0]][] = 'Apologies, at row number '.$this->row_num.", it seems that the 'location_code' column data has changed but the commission is already paid out, therefore we cannot change it now.";
                                }
                            }
                        }
                        if (strtoupper($row[7]) != strtoupper($sale->customer_state)) {
                            if (UserCommission::where(['pid' => $sale->pid, 'amount_type' => 'm2', 'status' => '3', 'settlement_type' => 'during_m2', 'is_displayed' => '1'])->first()) {
                                $errors['columns'][] = 'Apologies, at row number '.$this->row_num.", it seems that the 'customer_state' column data has changed but the commission is already paid out, therefore we cannot change it now.";
                                if (! $this->validate_only) {
                                    $this->salesErrorReport[$row[0]][] = 'Apologies, at row number '.$this->row_num.", it seems that the 'customer_state' column data has changed but the commission is already paid out, therefore we cannot change it now.";
                                }
                            }
                        }
                    } else {
            ...
            if (empty($errors['columns'])) {
                ...
                if (! $this->validate_only) {
                    ...
                    $val['rep_name'] = isset($row[12]) ? trim($row[12]) : null;
                    $val['rep_email'] = isset($row[13]) ? trim(strtolower($row[13])) : null;
