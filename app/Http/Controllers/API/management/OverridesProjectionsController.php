...
class OverridesProjectionsController extends Controller
{
    ...
    public function list_overrides(Request $request)
    {
        $process_status = 0;
        ...
        if ($filterDataDateWise == 'this_week') {
            ...
            $endDate = date('Y-m-d', strtotime(Carbon::now()->endOfWeek()));
            ...
            $endDate = date('Y-m-d', strtotime(Carbon::now()->subMonths(3)->endOfMonth()));
        ...
        if ($projected == 0) { // NORMAL OVERRIDE PART
            $data = UserOverrides::with('userInfo:id,first_name,last_name,image', 'salesDetail:pid,product,customer_name,m2_date');
            // Advance search,
            if (! empty($search)) {
            ...
            $sales->getCollection()->transform(function ($d) use ($companyProfile) {
                ...
                $product = Products::select('id', 'name', 'product_id')->where('id', $d->salesDetail->product)->first();
                ...
                return [
                    ...
                    'date' => date('m/d/Y', strtotime($d->salesDetail->m2_date)),
                ];
            });
    ...
    /**
     * @method my_overrides_cards
     * This method is used to get all override's count with their earnings
     */
    public function my_overrides_cards(Request $request)
    {
        $id = $request->input('id');
    ...
    /**
     * @method my_overrides_graph
     * This method is used to get all override's count with their earnings to show in graph
     */
    public function my_overrides_graph(Request $request)
    {
        $id = $request->input('id');
        $user = User::where('id', $id)->first();
