...
class ReconController extends Controller
{
        ...
        try {
            ...
            $data = DB::table('reconciliation_finalize_history')
                ->select(
                    ...
                    DB::raw('SUM(override) AS total_overrides'),
                    DB::raw('SUM( commission + override) AS total_due'),
                    DB::raw('SUM( percentage_pay_amount) AS percentage_pay_amount'),
                    // DB::raw('SUM(ABS(override)) AS total_overrides'),
                    // DB::raw('SUM(commission + override) AS total_due'),
                    // DB::raw('SUM(CASE WHEN commission > 0 THEN commission ELSE 0 END) AS total_commission'),
                    // DB::raw('SUM(CASE WHEN override > 0 THEN override ELSE 0 END) AS total_overrides'),
                    // DB::raw('SUM(CASE WHEN commission > 0 THEN commission ELSE 0 END + CASE WHEN override > 0 THEN override ELSE 0 END) AS total_due'),
                    DB::raw('SUM(clawback) AS total_clawback'),
                    DB::raw('SUM(gross_amount) AS gross_amount'),
                    // DB::raw('SUM(CASE WHEN clawback > 0 THEN clawback ELSE 0 END) AS total_clawback'),
                    DB::raw('SUM(adjustments) AS total_adjustments'),
                    DB::raw('SUM(deductions) AS total_deductions'),
                    DB::raw('MAX(payout) AS payout_percentage'),
            ...
            $data->transform(function ($item) {
                ...
                $total_override = $item->total_overrides;

                $totalPay = $total_commission + $total_override;
                $pays = $totalPay ? ($totalPay * $item->payout_percentage) / 100 : 0;
                if ($item->total_overrides < 0.5 && $item->total_commission < 0.5) {
                    $pays = 0;
                } else {
                    if ($item->total_overrides < 0.5) {
                        $pays = (($item->total_commission * $item->payout_percentage) / 100);
                    }

                    if ($item->total_commission < 0.5) {
                        $pays = (($item->total_overrides * $item->payout_percentage) / 100);
                    }

                    if ($item->total_commission > 0.5 && $item->total_overrides > 0.5) {
                        $pays = ($item->total_commission + $item->total_overrides) * ($item->payout_percentage / 100);
                    }
                }
                ...
                return [
                    ...
                    'overrides' => $total_override,
                    'total_due' => $totalPay,
                    'pays' => $item->percentage_pay_amount,
                    'clawback' => -1 * ($item->total_clawback),
                    'adjustments' => $item->total_adjustments + $item->total_deductions,
                    'payout' => $item->gross_amount,
                    'payout_per' => $item->payout_percentage,
                    'status' => $item->statuses,
                    'finalize_count' => $item->finalize_count,
                ];
            });
            // $totals = $this->calculateTotals();

            if ($request->has('sort')) {
                $data = sortData($data, $request->input('sort'), $request->input('sort_val'));
                // /* $data1 = $this->sortData($data, $request->input('sort'), $request->input('sort_val')); */
            }
            $data = pagination($data->toArray(), $perPage);
            ...
            Log::channel('reconLog')->info(print_r([
                'finalizeReconDraftList' => 'Error: '.$th->getMessage().' At line number '.$th->getLine(),
            ], true));
    ...
    public function reconCompanySettingStatus(Request $request)
    {
        ...
        try {
            ...
            if ($status) {
                ...
                if ($positionReconciliations && ! $request->status && $payrollReconciliations > 0) {
                    return response()->json([
    ...
    public function finalizeReconciliationList(Request $request)
    {
        $validate = Validator::make($request->all(), [
            ...
            // "finalize_count" => ["required", "numeric", "min:1"],
        ]);
        if ($validate->fails()) {
            return response()->json([
        ...
        $reconPayout = $request->recon_payout ?? 10;
        $sortBy = $request->sort;
        $sortOrder = $request->sort_val;
        $data = ReconciliationFinalizeHistory::join('users', 'reconciliation_finalize_history.user_id', '=', 'users.id')
            ->whereDate('reconciliation_finalize_history.start_date', $request->start_date)
            ->whereDate('reconciliation_finalize_history.end_date', $request->end_date)
            ->where('reconciliation_finalize_history.status', 'finalize')
            ...
            ->select('reconciliation_finalize_history.*');

        // /* $userIds = $this->getReconUserIds($officeIds, $positionIds, $search); */
        ...
        $userPosition = User::with(['positionDetail.payFrequency.frequencyType'])
            ->whereIn('id', $userIds)
            ->get()
            ->map(function ($user) {
                return [
        ...
        $reconWithHoldResponseArray = $this->getReconData($userIds, $startDate, $endDate, $reconPayout, $search, $request);
        switch ($sortBy) {
            case 'commission':
        ...
        // $filteredArray = array_filter($reconWithHoldResponseArray, function ($item) {
