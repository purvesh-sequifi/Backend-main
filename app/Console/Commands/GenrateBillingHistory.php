...
class GenrateBillingHistory extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'genratebilling:history';
     ...
     * @var string
     */
    protected $description = 'Function for Genrate Stripe Billing History';
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            if (isset($profiledata->is_flat) && $profiledata->is_flat == 1) {
                ...
                // For Aveyo billing of sclearance with flat rate
                if (config('app.domain_name') == 'aveyo' || config('app.domain_name') == 'aveyo2') {
                    $subscription_last = Subscriptions::with('plans', 'billingType')->where('status', 1)->where('plan_id', 2)->get();
                    if (! empty($subscription_last)) {
                        foreach ($subscription_last as $subscription_last) {
                            $this->SClearanceBilling($subscription_last, $invoice_no);
            ...
            $subscription_last = Subscriptions::with('plans', 'billingType')->where('status', 1)->where('flat_subscription', 0)->get();
            if (count($subscription_last) > 0) {
                ...
                foreach ($subscription_last as $subscription_last) {
                    ...
                    if ($plan_id == 1) {
                        ...
                        if ($evereeSubscriptionBillingHistory) {
                            ...
                            if ($update_status) {
                                ...
                                foreach ($payroll_histry_id_data as $pid_data_row) {
                                    ...
                                    $insert_SalesInvoiceDetail->customer_name = isset($pid_data_row['usersdata']) ? $pid_data_row['usersdata']['first_name'].' '.$pid_data_row['usersdata']['last_name'] : null;
                                ...
                                foreach ($one_time_payment_date_data as $otp_row) {
                                    ...
                                    $insert_SalesInvoiceDetail->customer_name = isset($otp_row['usersdata']) ? $otp_row['usersdata']['first_name'].' '.$otp_row['usersdata']['last_name'] : null;
                        ...
                        // SequiAI
                        $this->SquiAiBilling($subscription_last, $invoice_no);
                    } elseif ($plan_id == 7) {
                        Arena::SequiArenaBilling($subscription_last, $invoice_no);
                    } elseif ($plan_id == 6) {
                        Buckets::SequiCRMBilling($subscription_last, $invoice_no);
                    }
                }
            } else {
     ...
     */
    public function SClearanceBilling($subscription_last, $invoice_no)
    {
        DB::beginTransaction();
        ...
        $totalData = app(\App\Http\Controllers\API\StripeBillingController::class)->SClearanceData($subscription_last);

        $last_month_total = $totalData['total'];
    ...
    public function userWiseBilling($subscription_last, $invoice_no)
    {
        DB::beginTransaction();
        $plan_id = isset($subscription_last->plans) ? $subscription_last->plans->id : null;
        $plan_name = isset($subscription_last->plans) ? $subscription_last->plans->name : null;
    ...
    public function SquiAiBilling($subscription_last, $invoice_no)
    {
        DB::beginTransaction();
        $unique_pid_discount_price = 0;
        $m2_discount_price = 0;
