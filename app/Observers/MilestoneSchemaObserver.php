...
class MilestoneSchemaObserver
{
    ...
    /**
     * Handle the MilestoneSchema "created" event.
     */
    public function created(MilestoneSchema $milestoneSchema): void
    {
        ...
        $this->auditLog->addChange([
            'type' => get_class($milestoneSchema),
            'reference_id' => $milestoneSchema->id,
            'event' => 'created',
    ...
    /**
     * Handle the MilestoneSchema "updated" event.
     */
    public function updated(MilestoneSchema $milestoneSchema): void
    {
        // Retrieve the company's applicable timezone
        $timezone = $this->getCompanyTimezone();

        $changes = $milestoneSchema->getChanges();

        $logData = [];
        foreach ($changes as $key => $newValue) {
            $oldValue = $milestoneSchema->getOriginal($key);
    ...
    /**
     * Handle the MilestoneSchema "deleted" event.
     */
    public function deleted(MilestoneSchema $milestoneSchema): void
    {
        //
    }

    /**
     * Handle the MilestoneSchema "restored" event.
     */
    public function restored(MilestoneSchema $milestoneSchema): void
    {
        //
    }

    /**
     * Handle the MilestoneSchema "force deleted" event.
     */
    public function forceDeleted(MilestoneSchema $milestoneSchema): void
    {
        //
    }
}
