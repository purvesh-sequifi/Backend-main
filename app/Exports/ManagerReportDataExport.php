...
class ManagerReportDataExport implements FromCollection, WithHeadings
{
    private $startDates;
    ...
    public function __construct($startDate = 0, $endDate = 0, $stateCode = 0)
    {
    ...
    public function collection(): Collection
    {
        if ($this->startDates != 0 && $this->endDates != 0) {
            $records = SalesMaster::with('salesMasterProcess')->where('customer_state', $this->stateCode)->whereBetween('customer_signoff', [$this->startDates, $this->endDates])->get();
        } else {
            $records = SalesMaster::with('salesMasterProcess')->where('customer_state', $this->stateCode)->get();
        }
